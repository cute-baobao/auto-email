# auto-email Auto-Start Backend + /api/health — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make `auto-email` auto-start the backend — probe the port, reuse a running server or spawn one (logging to `~/.bao-auto-mail/server.log`), wait for readiness, then render the TUI and kill the spawned server on exit — plus a dedicated `GET /api/health` probe endpoint.
...
**Spec:** `docs/2026-07-04-auto-email-autostart-server-design.md`. Run from repo root. Existing 40 tests stay green.
...
```bash
git add packages/cli/src/index.tsx && git commit -m "feat(cli): auto-start backend on auto-email launch"
```
...
- [ ] **Step 2 (user, needs real `.env` + TTY):** Live checks:
  - `auto-email` alone → server auto-starts (first launch a few seconds), then TUI; `~/.bao-auto-mail/server.log` has server output; after quitting, `lsof -ti:45678` is empty (spawned server killed).
  - `bun run dev:server` first, then `auto-email` → reuses it (no second process), and quitting `auto-email` leaves the dev server running.
  - Verify path resolution works for the globally-linked `auto-email` (bin symlink) — if `../../server` doesn't resolve (symlink not followed), switch to walking up from `HERE` to the workspace root that contains `packages/server`. (Spec §7 risk.)
